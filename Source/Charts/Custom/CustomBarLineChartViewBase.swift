//
//  CustomBarLineChartViewBase.swift
//  Charts
//
...
open class CustomBarLineChartViewBase: ChartViewBase, BarLineScatterCandleBubbleChartDataProvider, NSUIGestureRecognizerDelegate
{
    ...
    internal var _xAxisRenderer: CustomXAxisRenderer!
    ...
    internal override func initialize()
    {
        ...
        _xAxisRenderer = CustomXAxisRenderer(viewPortHandler: _viewPortHandler, xAxis: _xAxis, transformer: _leftAxisTransformer)
        ...
        #if !os(tvOS)
            _pinchGestureRecognizer = NSUIPinchGestureRecognizer(target: self, action: #selector(CustomBarLineChartViewBase.pinchGestureRecognized(_:)))
        ...
        else if recognizer.state == NSUIGestureRecognizerState.ended || recognizer.state == NSUIGestureRecognizerState.cancelled
        {
            if _isDragging
            {
                if recognizer.state == NSUIGestureRecognizerState.ended && isDragDecelerationEnabled
                {
                    ...
                    _decelerationDisplayLink = NSUIDisplayLink(target: self, selector: #selector(CustomBarLineChartViewBase.decelerationLoop))
    ...
    public func gestureRecognizerShouldBegin(gestureRecognizer: NSGestureRecognizer) -> Bool
{
    ...
    /// **default**: An instance of CustomXAxisRenderer
    /// - returns: The current set X axis renderer
    open var xAxisRenderer: CustomXAxisRenderer
        {
    ...
    open func setLabelList(list: [String]?) {
        if let list = list {
            _xAxisRenderer.labelList = list
        }
    }
